...
WHATS_NEW = {
    "version": __version__,
    "features": [
        ("Automatic Export Detection", "Scans for exported audio files during project scanning with smart fuzzy matching (exact → normalized → fuzzy)"),
        ("Click-to-Play Exports", "Single-click project cards with exports to instantly play audio; click again to cycle through multiple exports"),
        ("In-App Audio Playback", "Play exported audio files from project details with full transport controls (WAV, AIFF, MP3, FLAC, OGG, M4A)"),
        ("Backup Project Management", "Backup .als files automatically excluded from grid; view and launch backups from Project Properties dialog"),
        ("Missing Project Detection", "Automatically marks missing projects; toggle View Missing Projects in View menu; cleanup tools in Tools menu"),
        ("Duplicate Detection", "Find duplicate projects using SHA256 hash comparison, name similarity, and location-based detection"),
        ("Tempo Filtering & Sorting", "Filter projects by tempo range (60-90, 90-120, 120-150, 150+ BPM or custom range) with always-visible controls"),
        ...
        ("MCP Agents Links", "Sidebar section with links to popular Ableton MCP server projects for AI integration"),
