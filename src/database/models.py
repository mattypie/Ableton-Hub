...
class Collection(Base):
    ...
    def projects(self) -> List["Project"]:
        ...
        return [pc.project for pc in sorted(
...
class Project(Base):
    ...
    arrangement_duration_seconds = Column(Float, nullable=True)  # Calculated duration in seconds (bars * 4 / tempo * 60)
    ...
    def get_key_display(self) -> Optional[str]:
    ...
    def get_live_version_major(self) -> Optional[int]:
    ...
    def get_live_version_display(self) -> Optional[str]:
...
class ProjectCollection(Base):
    ...
    def __repr__(self) -> str:
        return f"<ProjectCollection(project_id={self.project_id}, collection_id={self.collection_id}, track={self.track_number})>"
...
class LiveInstallation(Base):
    ...
    is_auto_detected = Column(Boolean, default=False)  # True if auto-detected, False if manually added
...
class AppSettings(Base):
    ...
    @classmethod
    def set_value(cls, session, key: str, value, value_type: str = "string", description: str = None):
        """Set a setting value."""
        setting = session.query(cls).filter(cls.key == key).first()
        if not setting:
