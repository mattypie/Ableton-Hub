...
class ArchiveService(QObject):
    ...
    def _copy_project_folder(self, source_folder: Path, backup_dir: Path, backup_name: str) -> str:
            ...
            if os.path.isdir(src):
                ...
                if copied_files % 10 == 0 or copied_files == total_files:
                    self.progress_updated.emit(
                        copied_files, total_files, f"Copying: {os.path.basename(src)}"
                    )
    ...
    def _create_zip_archive(self, source_folder: Path, backup_dir: Path, backup_name: str) -> str:
            ...
            for file_path in all_files:
                if file_path.is_file():
                    ...
                    if archived_files % 10 == 0 or archived_files == total_files:
                        self.progress_updated.emit(
                            archived_files, total_files, f"Archiving: {file_path.name}"
                        )
