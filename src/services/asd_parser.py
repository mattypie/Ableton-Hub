...
class ASDParser:
        ...
        for offset in search_offsets:
            if offset + 16 <= len(data):
                try:
                    ...
                    if (0 <= beat_time < 100000 and 
                        0 <= sample_time < 100000000 and
                        beat_time > 0 or sample_time > 0):
    ...
    def _extract_bpm(self, data: bytes) -> Optional[float]:
    ...
    def _extract_loop_info(self, data: bytes) -> Optional[LoopInfo]:
        """Extract loop information from ASD data."""
        # Loop info location varies by version
        # This is a placeholder for future implementation
        return None
    
    def _extract_sample_rate(self, data: bytes) -> Optional[float]:
