...
from ..database import Export, Project, get_session
...
class AudioPreviewGenerator:
    ...
    @staticmethod
    def _get_project_color(project_id: int, color_mode: COLOR_MODES = "rainbow") -> QColor | None:
    ...
    def has_ffmpeg() -> bool:
        ...
        try:
            subprocess.run(
                ["ffmpeg", "-version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=2
            )
            return True
    ...
    def generate_waveform_image(
        ...
        try:
            ...
            cmd = [
                ...
                str(audio_file),
                "-filter_complex",
                f"[0:a]aformat=channel_layouts=mono,showwavespic=s={width}x{height}:colors={color_hex}",
                "-frames:v",
                "1",
                "-y",  # Overwrite output
                str(output_file),
    ...
    def get_or_generate_preview(
        project_id: int, color_mode: COLOR_MODES | None = None, force_regenerate: bool = False
    ) -> str | None:
        ...
        try:
            ...
            if (
                ...
                and Path(project.thumbnail_path).exists()
            ):
            ...
            exports = (
                ...
                .order_by(Export.created_date.desc())
                .all()
            )
            ...
            if AudioPreviewGenerator.generate_waveform_image(
                str(export_path), str(preview_path), color_mode=color_mode, project_id=project_id
            ):
                ...
                pixmap = AudioPreviewGenerator.generate_simple_waveform(
                    str(export_path), color_mode=color_mode, project_id=project_id
                )
                if pixmap:
