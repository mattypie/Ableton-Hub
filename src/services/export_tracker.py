...
from ..utils.fuzzy_match import calculate_similarity, match_export_to_project
...
class ExportTracker(QObject):
    ...
    def __init__(self, parent: QObject | None = None):
        ...
        self._scanner: ExportScanner | None = None
    ...
    def scan_exports(self, project_id: int | None = None) -> None:
        ...
        try:
            ...
            for project in (
                session.query(Project).filter(Project.export_song_name.isnot(None)).all()
            ):
    ...
    def add_export(self, export_path: str, project_id: int | None = None) -> int | None:
        ...
        try:
            ...
            existing = session.query(Export).filter(Export.export_path == export_path).first()
    ...
    def auto_match_exports(self, threshold: float = 70.0) -> int:
        ...
        try:
            ...
            unlinked = session.query(Export).filter(Export.project_id.is_(None)).all()
