...
from .similarity_analyzer import SimilarityAnalyzer, SimilarProject
...
class RecommendationEngine:
    ...
    def recommend_similar_projects(self,
        ...
        recommendations = []
        for sim in similar:
            reason = self._similarity.get_similarity_explanation(
                sim.similarity_result
            ) if sim.similarity_result else "Similar project structure"
            ...
            recommendations.append(Recommendation(
                ...
                metadata={
                    ...
                    'similarity_breakdown': (
                        sim.similarity_result.to_dict() 
                        if sim.similarity_result else {}
                    )
                }
            ))
    ...
    def recommend_plugins(self,
        ...
        if cooccurrence_scores:
            ...
            for plugin, count in cooccurrence_scores.most_common(n_recommendations):
                ...
                # Build reason
                common_with = [
                    p for p in current_plugins 
                    if self._plugin_cooccurrence[p].get(plugin, 0) > 0
                ][:3]
        ...
        if cooccurrence_scores:
            ...
            for device, count in cooccurrence_scores.most_common(n_recommendations):
                ...
                common_with = [
                    ...
                    if self._device_cooccurrence[d].get(device, 0) > 0
                ][:3]
    ...
    def _suggest_structure_tags(self,
                                 track_count: int,
                                 tag_lookup: Dict[str, Dict]
                                 ) -> List[Recommendation]:
