...
class RemoteSync(QObject):
    ...
    def detect_dropbox_folder(self) -> Path | None:
    ...
    def detect_onedrive_folder(self) -> Path | None:
    ...
    def detect_google_drive_folder(self) -> Path | None:
    ...
    def detect_icloud_folder(self) -> Path | None:
    ...
    def detect_ableton_cloud_folder(self) -> Path | None:
    ...
    def detect_all_cloud_folders(self) -> dict[str, Path | None]:
    ...
    def create_location_from_cloud(self, service: str) -> int | None:
        ...
        try:
            ...
            existing = session.query(Location).filter(Location.path == str(path)).first()

            if existing:
                return existing.id

            # Create location
            location = Location(name=name, path=str(path), location_type=loc_type, is_active=True)
