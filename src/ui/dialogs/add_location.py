...
class AddLocationDialog(QDialog):
    ...
    def __init__(self, parent=None, location_id: int | None = None):
        ...
        if not self.location_id:
            ...
            if suggestions:
                ...
                for path in suggestions[:5]:  # Show max 5
                    ...
                    """)
                    suggest_btn.clicked.connect(lambda checked, p=path: self._use_suggestion(p))
    ...
    def _browse_folder(self) -> None:
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Select Location Folder", self.path_input.text() or str(Path.home())
        )
        if folder:
