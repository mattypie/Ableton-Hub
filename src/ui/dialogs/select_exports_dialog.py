    ...
    def _load_existing_exports(self) -> None:
        ...
        try:
            ...
            all_exports = (
                ...
                .order_by(
                    Export.project_id.is_(None).desc(), Export.export_date.desc()  # Unlinked first
                )
                .all()
            )
    ...
    def _link_selected_existing(self) -> None:
        ...
        if not selected_items:
            QMessageBox.information(
                self, "No Selection", "Please select one or more exports from the list."
            )
            return
        ...
        try:
            ...
            if linked_count > 0:
                session.commit()
                QMessageBox.information(
                    self, "Exports Linked", f"Linked {linked_count} export(s) to this project."
                )
                self._load_existing_exports()
                ...
                self._update_selected_list()
                QMessageBox.information(
                    self, "Exports Selected", "Selected exports are already linked to this project."
                )
        finally:
            session.close()
        ...
        try:
            ...
            if linked_count > 0:
                ...
                if self.selected_list.count() > 0:
                    ...
                    QMessageBox.information(
                        self, "No Selection", "Please select exports to link or browse for files."
                    )
        finally:
            session.close()
