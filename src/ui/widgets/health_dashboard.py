...
class HealthDashboard(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        health_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        health_section.addWidget(health_label)

        self.excellent_bar = QProgressBar()
        self.excellent_bar.setFormat("Excellent: %p%")
        self.excellent_bar.setStyleSheet(
            f"QProgressBar::chunk {{ background-color: {AbletonTheme.COLORS['success']}; }}"
        )
        health_section.addWidget(self.excellent_bar)

        self.good_bar = QProgressBar()
        self.good_bar.setFormat("Good: %p%")
        self.good_bar.setStyleSheet(
            f"QProgressBar::chunk {{ background-color: {AbletonTheme.COLORS['accent']}; }}"
        )
        health_section.addWidget(self.good_bar)

        self.fair_bar = QProgressBar()
        self.fair_bar.setFormat("Fair: %p%")
        self.fair_bar.setStyleSheet(
            f"QProgressBar::chunk {{ background-color: {AbletonTheme.COLORS['warning']}; }}"
        )
        health_section.addWidget(self.fair_bar)

        self.poor_bar = QProgressBar()
        self.poor_bar.setFormat("Poor: %p%")
        self.poor_bar.setStyleSheet(
            f"QProgressBar::chunk {{ background-color: {AbletonTheme.COLORS['error']}; }}"
        )
        ...
        issues_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        ...
        duplicates_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        layout.addWidget(duplicates_label)

        self.duplicates_table = QTableWidget()
        self.duplicates_table.setColumnCount(3)
        self.duplicates_table.setHorizontalHeaderLabels(["Type", "Projects", "Actions"])
        ...
        try:
            ...
            for row, (project, health) in enumerate(issues):
                ...
                view_btn.clicked.connect(
                    lambda checked, pid=project.id: self.project_selected.emit(pid)
                )
    ...
    def _update_duplicates_table(self) -> None:
        ...
        for row, (dup_type, project_ids, _info) in enumerate(rows):
