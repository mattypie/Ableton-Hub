...
from ...database import get_session, LinkDevice
...
class DeviceCard(QFrame):
    ...
    def __init__(self, device: LinkDevice, parent: Optional[QWidget] = None):
    ...
    def _setup_ui(self) -> None:
        ...
        if self.device.last_seen:
            ...
            seen_label.setStyleSheet(f"color: {AbletonTheme.COLORS['text_secondary']}; font-size: 10px;")
...
class LinkPanel(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        devices_label.setStyleSheet(f"font-size: 16px; font-weight: bold;")
    ...
    def _on_device_found(self, device_name: str, ip_address: str, port: int) -> None:
        ...
        try:
            ...
            device = session.query(LinkDevice).filter(
                LinkDevice.device_name == device_name,
                LinkDevice.ip_address == ip_address
            ).first()
    ...
    def _refresh_devices(self) -> None:
        ...
        session = get_session()
        try:
            self._devices = session.query(LinkDevice).order_by(
                LinkDevice.is_active.desc(),
                LinkDevice.last_seen.desc()
            ).all()
            ...
            self.devices_count_label.setText(f"Devices: {online} online, {len(self._devices)} total")
        finally:
            session.close()
